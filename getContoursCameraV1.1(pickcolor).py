...
pixel_filter = 10 # Minimum pixel area to filter noise
...
def measurement_mode():
    ...
    # Tentacle Detection with Adaptive Illumination Compensation
    ...
    # Use equalized HSV for masking
    if picked_hsv is not None:
        lower_color = np.maximum(picked_hsv - hsv_tolerance, [0, 60, 0])
        upper_color = np.minimum(picked_hsv + hsv_tolerance, [179, 255, 180])
    else:
    ...
    mask = cv2.inRange(hsv_eq, lower_color, upper_color)
    mask = cv2.erode(mask, None, iterations=1)
    mask = cv2.dilate(mask, None, iterations=2)
    mask = cv2.medianBlur(mask, 7)  # Add this line for extra smoothing
    ...
    # Show live picked HSV if color was picked
    if picked_hsv is not None:
        cv2.putText(frame, f"HSV picked: {picked_hsv}", (10, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

    cv2.imshow("Tentacle Mask", mask)
